...
def get_args():
    ...
    parser.add_argument('--exp_dir', nargs="?", type=str, default="/home/qwang/rob/temp", help='Folder of the experiment')
    
    parser.add_argument('--save_model', nargs="?", type=str2bool, default=False, help='Save model.pth.tar with best loss')
    parser.add_argument('--stop_p', nargs="?", type=int, default=1, help='Number of cases when valid loss and F1 do not meet criteria')
    parser.add_argument('--stop_c1', nargs="?", type=float, default=0.008, help='Acceptable difference compared with the best loss')
    parser.add_argument('--stop_c2', nargs="?", type=float, default=0.008, help='Acceptable difference compared with the best f1')
...
#    parser.add_argument('--tune_embed', nargs="?", type=str2bool, default=True, help='Tune embedding or not')
    ...
    parser.add_argument('--max_doc_len', nargs="?", type=int, default=99999, help='Maximum number of sents in one document overall the batches')
    
    # CNN
    parser.add_argument('--num_filters', nargs="?", type=int, default=100, help='Number of filters for each filter size (cnn)')   
    parser.add_argument('--filter_sizes', nargs="?", type=str, default='7,8,9', help='Filter sizes (cnn)')
